...
# XXD Panel 185｜ملصق أحادي اللون بلمسة فاتحة
...
- `design-only`: تعرض اللوحة الكاملة ترجمة Panel 185 التصميمية فقط، وتبقى الصورة مرجعاً غير ظاهر.
...
بعد التثبيت أعد تشغيل جلسة Agent ثم استدعِ `$xxd-panel-185`. ويمكن إضافة `--global --agent codex --yes` للتثبيت على مستوى المستخدم.

```text
/xxd-panel-185 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ar-SA
...
راجع [SKILL.md](SKILL.md) لعقد التشغيل الكامل، ومهايئ التشغيل [بالإنجليزية](references/xxd-panel-185-prompt.en.md) أو [بالصينية](references/xxd-panel-185-prompt.zh-CN.md).
