...
# XXD Panel 185｜옅은 강조색의 흑백 스티커 소품
...
- `design-only`: 전체 캔버스에 Panel 185의 디자인 번역만 표시하고 사진은 보이지 않는 참고 자료로 사용합니다.
...
설치 후 Agent 세션을 다시 시작하고 `$xxd-panel-185`을 호출하세요. 사용자 단위 Codex 설치에는 `--global --agent codex --yes`를 추가할 수 있습니다.

```text
/xxd-panel-185 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ko-KR
...
전체 실행 계약은 [SKILL.md](SKILL.md), 런타임 어댑터는 [영어](references/xxd-panel-185-prompt.en.md)와 [중국어](references/xxd-panel-185-prompt.zh-CN.md)를 확인하세요.
